...
export default function SemesterDateStep() {
  ...
  const semesterMutation = useMutation({
    ...
    mutationFn: () => execute(CREATE_SEMESTER_MUTATION, { input: { totalEcts: 0, startDate: formData.startDate?.toISOString().split("T")[0], endDate: formData.endDate?.toISOString().split("T")[0], semester: formData.semester as number } })
  })
  ...
  const isDisabled = !formData.semester || !formData.startDate || !formData.endDate
  return (
    <>
