...
export default function CoursesCard({ course }: { course: Course }) {
	...
	const updateMutation = useMutation({
		...
		mutationFn: (updatedCourse: NewCourse) =>
			execute(UPDATE_COURSE_MUTATION, { input: updatedCourse }),
		onSuccess: () => {
	...
	const deleteMutation = useMutation({
		...
		mutationFn: (deleteCourse: Course) =>
			execute(DELETE_COURSE_MUTATION, { id: deleteCourse.id }),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["courses_index"],
				exact: true,
	...
	const handleUpdate = (updatedCourse: Course) => {
		...
		toast.promise(mut, {
			...
			error: (data) =>
				`Error occured while updating ${data.updateCourse.name}}`,
		});
	};
				...
				<CardContent>
					<div className="space-y-4">
						<div className="flex items-center justify-between">
							<div className="flex items-center ">
								...
								<span className="text-sm text-muted-foreground">
									{course.todos[0]
										? `${new Date(course.todos[0].dueOn).toLocaleDateString()} : ${course.todos[0].title}`
										: "No upcoming todos"}
								</span>
							</div>
						...
						<div className="flex items-center">
							...
							<span className="text-sm text-muted-foreground">
								Grade: {course.grade ? course.grade : "not set yet"}
							</span>
						</div>
						<div className="flex items-center">
