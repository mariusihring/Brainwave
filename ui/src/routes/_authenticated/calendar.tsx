...
import { differenceInMinutes, format, isSameDay, parseISO, addMinutes, setHours, setMinutes } from "date-fns";
...
function CalendarIndex() {
  ...
  const onDragUpdate = (update) => {
    ...
    const draggedAppointment = localAppointments.find((app) => app.id === draggableId);
    ...
    const newStartTime = setHours(setMinutes(newDay, start.getMinutes()), start.getHours());
    const durationMinutes = differenceInMinutes(parseISO(draggedAppointment.endTime), start);
  ...
  const onDragEnd = (result) => {
    ...
    const updatedAppointments = localAppointments.map((appointment) =>
      appointment.id === draggedAppointment.id
        ? { ...appointment, startTime: newStartTime.toISOString(), endTime: newEndTime.toISOString() }
        : appointment
    );
          ...
          <div className="grid grid-cols-6 gap-4">
            ...
            <div className="col-span-6">
              <div className="relative">
                {weekDays.map((day, index) => (
                  <Droppable key={day.toISOString()} droppableId={day.toISOString()} type="APPOINTMENT">
                    {(provided) => (
                      <div
                      ...
                      >
                        {previewStyle && destination?.droppableId === day.toISOString() && (
                          <div
                            ...
                            <Draggable key={appointment.id} draggableId={appointment.id} index={index}>
                              {(provided, snapshot) => (
                                <div
                                  ...
                                  style={{ ...getAppointmentStyle(appointment), ...provided.draggableProps.style }}
                                  onDoubleClick={() => handleResize(appointment.id, "end", 30)}
                                ...
                                >
                                  ...
                                  <div className="text-xs text-stone-600 mb-1">{appointment.location}</div>
                                  <div className="text-xs text-stone-500">{format(parseISO(appointment.startTime), "p")} - {format(parseISO(appointment.endTime), "p")}</div>
                                </div>
                              )}
                            </Draggable>
                          ))}
            ...
            {timeSlots.map((time) => (
              <React.Fragment key={time}>
                ...
                {weekDays.map((day) => (
                  <div key={`${day.toISOString()}-${time}`} className="relative h-12 group">
